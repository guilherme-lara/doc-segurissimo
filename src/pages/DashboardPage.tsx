...
const DashboardPage = () => {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`dashboard-rt-${company.id}`)
  ...
  const addDocumentTag = (label: string, stage: string, type: "file" | "text" = "file") => {
    if (checklistItems.some(i => normalize(i.itemName) === normalize(label))) {
      toast.warning("Item já na lista!");
      return;
    }
  ...
  const applyTemplate = (tpl: any) => {
    ...
    tpl.items.forEach((t: any) => {
      if (!merged.some(m => normalize(m.itemName) === normalize(t.label))) {
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-5xl px-4 py-8">
        <div className="mb-8 flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Gerencie solicitações e arquivos</p>
          </div>
          <div className="flex gap-2 flex-wrap">
            <Button variant="outline" className="rounded-xl transition-all" onClick={() => navigate(`/${slug}/templates`)}>
              <LayoutList className="mr-1.5 h-4 w-4" /> Templates
            </Button>
            
            {isPro && owncloudUrl && (
              <Button variant="outline" size="sm" className="rounded-xl transition-all" onClick={() => setCloudSyncModalOpen(true)}>
            ...
            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
              <DialogTrigger asChild>
                <Button className="rounded-xl gradient-primary text-primary-foreground shadow-hero hover:shadow-glow transition-all">
              ...
              <DialogContent className="max-w-xl rounded-3xl overflow-y-auto max-h-[90vh]">
                <DialogHeader>
                  ...
                  <DialogDescription>Preencha os dados do cliente e selecione os documentos necessários.</DialogDescription>
                </DialogHeader>
                  ...
                  {checklistItems.length > 0 && (
                    <div className="flex flex-wrap gap-2 p-3 bg-accent/20 rounded-xl border border-dashed">
                      {checklistItems.map((it, i) => (
                        <Badge key={i} variant="secondary" className="rounded-full pr-1.5 py-1 text-[11px] gap-1.5 bg-background shadow-sm">
                          {it.itemName}
                          <button onClick={() => setChecklistItems(checklistItems.filter((_, idx) => idx !== i))} className="hover:text-destructive"><Trash2 className="h-3 w-3" /></button>
                        </Badge>
                      ))}
                    </div>
                  )}
                ...
                <DialogFooter className="mt-6 border-t pt-4">
                  <Button onClick={() => createRequest.mutate()} className="w-full rounded-xl gradient-primary h-11 shadow-hero">Gerar Link de Upload ✨</Button>
                </DialogFooter>
              </DialogContent>
            </Dialog>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
          {[
            ...
            <div key={i} className="rounded-2xl border border-border/60 bg-card p-4 shadow-card hover:shadow-elevated transition-all">
        ...
        <Tabs defaultValue="requests" className="w-full">
          ...
          <TabsContent value="requests" className="space-y-4">
             <div className="flex gap-2 mb-4">
                ...
                <Button variant="ghost" size="sm" onClick={() => setShowArchived(!showArchived)} className="ml-auto rounded-xl text-xs gap-1.5"><Archive className="h-3.5 w-3.5" /> {showArchived ? "Ver Ativas" : "Ver Engavetadas"}</Button>
             </div>
             
             {requestsLoading ? <Skeleton className="h-32 w-full rounded-2xl" /> : visibleRequests.map((req: any) => (
                <motion.div key={req.id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-2xl border bg-card p-5 shadow-sm hover:shadow-elevated transition-all mb-3">
                   <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground flex items-center gap-1.5"><Clock className="h-3 w-3" /> Criado em {new Date(req.created_at).toLocaleDateString("pt-BR")}</p>
                      </div>
                      <div className="flex gap-2">
                        <Button variant="outline" size="sm" onClick={() => { navigator.clipboard.writeText(`${window.location.origin}/${slug}/enviar/${req.id}`); toast.success("Link copiado!"); }} className="rounded-xl shadow-sm"><Copy className="h-3.5 w-3.5 mr-1.5" /> Link</Button>
          ...
          <TabsContent value="files">
             <div className="mb-4 relative">
                ...
                <Input placeholder="Buscar arquivos..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-10 rounded-xl bg-card" />
             </div>
             <div className="rounded-2xl border overflow-hidden shadow-sm bg-card">
                <Table>
                  <TableHeader className="bg-muted/30"><TableRow><TableHead>Arquivo</TableHead><TableHead>Cliente</TableHead><TableHead>Status</TableHead><TableHead className="text-right">Data</TableHead></TableRow></TableHeader>
                  <TableBody>
                    {(filteredUploads || []).map((file: any) => (
                      <TableRow key={file.id} className="cursor-pointer hover:bg-accent/40" onClick={() => { setPreviewFile(file); setPreviewOpen(true); }}>
                        ...
                        <TableCell>
                          <Badge variant="outline" className={`text-[10px] px-2 rounded-full font-bold ${file.status === 'approved' ? 'text-success border-success/30 bg-success/5' : file.status === 'rejected' ? 'text-destructive border-destructive/30 bg-destructive/5' : 'text-amber-600'}`}>{file.status?.toUpperCase()}</Badge>
                        </TableCell>
      ...
      <Dialog open={manageRequestOpen} onOpenChange={setManageRequestOpen}>
        <DialogContent className="max-w-sm rounded-3xl text-center p-8">
          <DialogHeader className="hidden"><DialogTitle>Gerenciar</DialogTitle><DialogDescription>Ações</DialogDescription></DialogHeader>
          <div className="space-y-6">
            ...
            <p className="text-sm text-muted-foreground">Gerenciar a solicitação de <b>{selectedRequest?.clientName}</b>?</p>
            <div className="flex flex-col gap-3">
              <Button variant="outline" className="rounded-xl h-11" onClick={() => selectedRequest && archiveRequest.mutate(selectedRequest.id)}><Archive className="h-4 w-4 mr-2" /> Engavetar</Button>
              <Button variant="destructive" className="rounded-xl h-11" onClick={() => selectedRequest && deleteRequest.mutate(selectedRequest.id)}><Trash2 className="h-4 w-4 mr-2" /> Excluir</Button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
